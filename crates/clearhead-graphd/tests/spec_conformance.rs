//! Opt-in conformance: graphd's real index query output validates against the
//! specification's published `index_query_result` schema.
//!
//! graphd owns query output, so schema conformance for those bytes is proven
//! here, at the producer boundary — the CLI forwarding shim never inspects the
//! payload. Runs only with `--features spec-conformance`; a default
//! `cargo test` stays independent of a `specifications` checkout.
...
/// Run `query index <view> --format json` against a seeded workspace and return
/// the parsed JSON array of rows.
...
/// Seed one workspace that exercises every optional column of the contract:
/// an undated parent/child pair (parent, parent_name, priority) surfaced by the
...
fn validator_rejects_a_row_missing_required_id() {
    ...
    // contract, this malformed row (no `id`, plus an unknown column) would
    // slip through and the conformance test above would prove nothing.
    let validator = index_validator();

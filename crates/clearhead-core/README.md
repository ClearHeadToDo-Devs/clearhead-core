...
This crate is one member of the `clearhead-rs` workspace; the delivery adapter
and the native hosts are its siblings under `crates/`. See the
[workspace README](../../README.md) for the full layout.
...
Without `formatting`, parsing and domain operations remain available, while attempts to format `.actions` source return a capability error. JSON and XML serialization do not require the feature.

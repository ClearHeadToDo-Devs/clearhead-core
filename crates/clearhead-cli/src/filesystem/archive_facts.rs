...
use crate::filesystem::{charter_root, workspace_data_root};
...
pub fn read_archived_action_facts(root: &Path) -> Result<Vec<ArchivedActionFact>, WorkspaceError> {
    ...
    for path in files {
        let metadata = crate::filesystem::sidecar::read_sidecar(&sidecar_path(&path))?;
        let source_path = path.strip_prefix(root).unwrap_or(&path).to_path_buf();
        for action in crate::filesystem::action_files::read_actions(&path)? {
...
mod tests {
    use super::*;
    use crate::filesystem::sidecar::write_sidecar;
    use chrono::TimeZone;

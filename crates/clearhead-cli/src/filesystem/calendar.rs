...
fn sync_plan_templates(
    ...
    for charter in &workspace.charters {
        ...
        for plan in &charter.plans {
            ...
            let Some(template_path) = crate::filesystem::templates::resolve_template(
                charter_dir,
                data_root,
                template_name,
            )?
            ...
            let steps = crate::filesystem::read_actions(&template_path)?;
...
pub fn apply_occurrence_op(
    ...
    crate::filesystem::execute_effects(&observation.mounts, effects.effects())
}
...
pub fn sync_master_rollforwards(
    ...
    let (store_source, store_expected) = match std::fs::read(&store_path) {
        Ok(bytes) => (
            ...
            ExpectedResource::Revision(crate::filesystem::mounts::content_revision(&bytes)),
        ),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
...
pub fn write_collection_displaynames(
        ...
        crate::filesystem::mounts::read_workspace(workspace_root, external_plans)?.charters
    {
        ...
        crate::filesystem::durability::atomic_write(&path, alias)?;
        refreshed += 1;
    }
    Ok(refreshed)
}
...
pub fn write_plan_file(
    ...
    .map_err(|error| WorkspaceError::Actions(error.to_string()))?;
    crate::filesystem::validate_preconditions(&mounts, effects.preconditions())?;
    crate::filesystem::execute_effects(&mounts, effects.effects())
}
...
pub fn delete_plan_file(
    ...
    let effects = EffectBatch::new(
        ...
        vec![ResourcePrecondition {
            path: location,
            expected: ExpectedResource::Revision(crate::filesystem::mounts::content_revision(
                &bytes,
            )),
        }],
    )

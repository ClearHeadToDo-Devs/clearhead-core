...
fn action_file_roundtrip_uses_core_codec_through_native_wrapper() {
    ...
    clearhead_cli::filesystem::write_actions(std::slice::from_ref(&action), &path).unwrap();
    assert_eq!(
        clearhead_cli::filesystem::read_actions(&path).unwrap(),
        vec![action]
    );
}
...
fn template_resolution_preserves_local_then_global_precedence() {
    ...
    assert_eq!(
        clearhead_cli::filesystem::templates::resolve_template(&charter, &data, "review").unwrap(),
        Some(local)
    );
}
...
fn sidecar_wrapper_applies_core_stamping_policy() {
    ...
    clearhead_cli::filesystem::sidecar::stamp_sidecar_entries(
        &actions_path,
        std::slice::from_ref(&action),
    )
    .unwrap();
    let path = clearhead_core::workspace::sidecar_path(&actions_path);
    let metadata: CharterMetadata =
        clearhead_cli::filesystem::sidecar::read_sidecar(&path).unwrap();
